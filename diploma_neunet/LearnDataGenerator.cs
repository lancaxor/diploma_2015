...
namespace diploma_neunet
{
    class LearnDataGenerator        //Generate vector for neunet learning: char to bitmap then to array
    {
        ...
        public static int[] Generate(char ch, int size)
        {
            r = new Random();
            ...
            for (int i = 0; i < (int)(size * size * currNoise / 100); i++)      // size --> 100%, => (size * currNoise / 100) -> currNoise%
            {
        ...
        private static void TestForm(Bitmap bmp)
        {
            Form tf = new Form();
